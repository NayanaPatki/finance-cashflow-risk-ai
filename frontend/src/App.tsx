    ...
    <main className="app-shell">
      <header className="command-header">
        <div className="product-lockup">
          <span className="product-mark" aria-hidden="true"><WalletCards size={22} /></span>
          <div><strong>CashLens</strong><span>Treasury &amp; Risk OS</span></div>
        </div>
        <div className="command-meta">
          <span className="model-chip">13-week model</span>
          <span className={`connection ${connection}`}><span className="status-dot" />{connection === "live" ? "Live API" : connection === "demo" ? "Demo fallback" : "Connecting"}</span>
          <button className="icon-button" type="button" aria-label="Refresh finance model" title="Refresh finance model" onClick={() => void refreshData()} disabled={refreshing}><RefreshCw size={18} className={refreshing ? "spin" : ""} /></button>
        </div>
      </header>

      <nav className="workspace-tabs" aria-label="Finance dashboard views" role="tablist" aria-orientation="horizontal">
        {navItems.map((item) => {
          const Icon = item.icon;
          return <button key={item.id} className={`workspace-tab${view === item.id ? " active" : ""}`} type="button" role="tab" aria-selected={view === item.id} aria-label={item.label} onClick={() => setView(item.id)}><Icon size={17} /><span>{item.label}</span></button>;
        })}
      </nav>

      <section className="workspace">
        <header className="page-heading">
          <div><p className="eyebrow">Model as of {summary.as_of_date} / USD</p><h1>{current.title}</h1><p className="page-subtitle">{current.subtitle}</p></div>
          <div className="page-context"><span>Active source</span><strong>{source.source_type === "demo" ? "Demo ledger" : "Imported CSV"}</strong></div>
        </header>
...
function Overview({ summary, forecast, receivables, anomalies }: { summary: FinanceSummary; forecast: ForecastWeek[]; receivables: ReceivableRisk[]; anomalies: ExpenseAnomaly[] }) {
  ...
  const endingCash = forecast[forecast.length - 1]?.ending_cash ?? 0;
  return <section className="treasury-board">
    <div className="liquidity-stage">
      <div className="stage-heading"><div><span className="section-index">01 / Liquidity</span><h2>Forward cash position</h2></div><StatusBadge status={summary.liquidity_status} /></div>
      <div className="stage-balance"><span>Available cash</span><strong>{money.format(summary.current_cash)}</strong><small className={summary.net_cash_flow_30d < 0 ? "negative-text" : "positive-text"}>{money.format(summary.net_cash_flow_30d)} net movement / 30d</small></div>
      <CashForecastChart forecast={forecast} buffer={summary.minimum_cash_buffer} />
      <div className="cash-movement-strip">
        <div><span>Opening</span><strong>{money.format(summary.current_cash)}</strong></div>
        <div className="positive"><span>Inflows</span><strong>+{money.format(totalInflow)}</strong></div>
        <div className="negative"><span>Outflows</span><strong>-{money.format(totalOutflow)}</strong></div>
        <div><span>Week 13 close</span><strong>{money.format(endingCash)}</strong></div>
      </div>
    </div>

    <aside className="position-ledger" aria-label="Finance position register">
      <div className="ledger-heading"><span className="section-index">02 / Position register</span><BarChart3 size={19} /></div>
      <div className="ledger-row"><span>30-day net cash</span><strong className={summary.net_cash_flow_30d < 0 ? "negative-text" : "positive-text"}>{money.format(summary.net_cash_flow_30d)}</strong><small>{summary.net_cash_flow_30d < 0 ? "Operating burn" : "Cash generation"}</small></div>
      <div className="ledger-row"><span>Runway</span><strong>{summary.cash_runway_months === null ? "N/A" : `${summary.cash_runway_months} mo`}</strong><small>Trailing 90-day basis</small></div>
      <div className="ledger-row"><span>Overdue AR</span><strong>{money.format(summary.overdue_receivables)}</strong><small>{percent.format(summary.overdue_receivables / Math.max(summary.open_receivables, 1))} of open receivables</small></div>
      <div className="ledger-row"><span>Reserve floor</span><strong>{money.format(summary.minimum_cash_buffer)}</strong><small>Management minimum</small></div>
      <div className="ledger-footer"><ShieldAlert size={17} /><span>Base case stays above reserve, but short runway keeps liquidity on watch.</span></div>
    </aside>

    <div className="decision-queue">
      <div className="queue-heading"><div><span className="section-index">03 / Decisions</span><h2>Finance action register</h2></div><span>3 open actions</span></div>
      <div className="queue-list">
        {highestRisk && <article><span className="queue-number">01</span><span className="action-icon amber"><ReceiptText size={17} /></span><div><strong>Resolve {highestRisk.customer} AR risk</strong><p>{highestRisk.recommended_action}. Exposure: {money.format(highestRisk.balance)}.</p></div><span className="queue-owner">AR owner</span></article>}
        <article><span className="queue-number">02</span><span className="action-icon red"><AlertTriangle size={17} /></span><div><strong>Review {anomalies.length} expense exceptions</strong><p>{anomalies[0]?.category ?? "Expense"} is {anomalies[0]?.amount_multiple ?? 0}x its category median.</p></div><span className="queue-owner">Controller</span></article>
        <article><span className="queue-number">03</span><span className="action-icon blue"><CalendarClock size={17} /></span><div><strong>Protect the reserve floor</strong><p>Review discretionary outflows while runway remains {summary.cash_runway_months ?? "unavailable"} months.</p></div><span className="queue-owner">Treasury</span></article>
      </div>
    </div>
  </section>;
}
